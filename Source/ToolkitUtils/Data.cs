...
public static partial class Data
{
    ...
    // Remove async methods and replace with synchronous versions that use LongEventHandler
    internal static T LoadJsonThreaded<T>(string path, bool ignoreErrors = false) where T : class
    {
        T result = null;
        ...
        LongEventHandler.QueueLongEvent(() =>
        {
            try
            {
                result = LoadJson<T>(path, ignoreErrors);
            }
            catch (Exception e)
            {
                threadException = e;
    ...
    // Replace async methods with threaded versions using LongEventHandler
    public static void SaveJsonThreaded<T>(T obj, string path) where T : class
    {
        LongEventHandler.QueueLongEvent(() =>
        {
            SaveJson(obj, path);
    ...
    private static void SaveCompressedJson<T>(T obj, string path) where T : class
    {
        ...
        try
        {
            using (FileStream file = File.Open(tempPath, FileMode.Create, FileAccess.Write))
            {
                ...
                {
                    Json.Serialize(zipStream, obj, !TkSettings.MinifyData);
                }
            }
        }
        catch (IOException e)
    ...
    /// </summary>
    /// <param name="path">The file to save shop data to</param>
    public static void SaveLegacyShopThreaded(string path)
    {
        LongEventHandler.QueueLongEvent(() =>
        {
            SaveLegacyShop(path);
    ...
    // Remove the async version and create a threaded version
    public static void DumpAllDataThreaded()
    {
        LongEventHandler.QueueLongEvent(() =>
        {
            DumpAllData();
    ...
    public static void SaveTraitsThreaded(string path)
    {
        LongEventHandler.QueueLongEvent(() =>
        {
            SaveTraits(path);
    ...
    public static void SavePawnKindsThreaded(string path)
    {
        LongEventHandler.QueueLongEvent(() =>
        {
            SavePawnKinds(path);
